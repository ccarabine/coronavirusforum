...
<!--
                    <div class="d-flex">
                        <span> {{ comments.count }} Comment{{ comments|pluralize:"s" }} </span>
                        <span id="votes" data-value="{{post.id}}"></span>
                        <div class="pr-2 ml-3 ">
                            <span id="upvote"><span id="up">{{post.upvote}}</span></span>
                            <a {% if request.user.is_authenticated %}
                                class="btn btn-link p-0 border-0 btn-outline-light thumbaction thumb" {% else %}
                                href="{% url 'account_login' %}"
                                class="btn btn-link txt-color p-0 border-0 btn-outline-light" {% endif %} value="upvote"
                                data-toggle="tooltip" title="I like this" role="button">
                                <svg id="upvote" width="1.5em" height="1.5em" viewBox="0 0 16 16"
                    ...
                    <<div class="pr-2">
                        <span id="downvote">{{post.downvote}}</span>
                        <a {% if request.user.is_authenticated %}
                            class="btn btn-link p-0 border-0 btn-outline-light thumbaction" {% else %}
                            href="{% url 'accounts:register' %}"
                            class="btn btn-link p-0 border-0 btn-outline-light" {% endif %} value="downvote"
                            data-toggle="tooltip" title="I dont like this" role="button">
                            <svg id="downvote" width="1.5em" height="1.5em" viewBox="0 0 16 16"
                                class="down bi bi-hand-thumbs-down" fill="currentColor"
